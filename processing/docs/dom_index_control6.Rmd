...
## Control time invariant variable (sex)

control_sex <- '
  DOM_INDEX1 + DOM_INDEX2 + DOM_INDEX3 + DOM_INDEX4 ~ s1*sexo1 # Constrained over time. 
  CS1 + CS2 + CS3 + CS4 ~ s2*sexo1 # Constrained over time. 
'
...
for (i in models){
  fit[[i]] <- lavaan(model = c(bwcomp,control_sex,get(i),varcov),
                     data = elsoc, 
                     estimator = "MLR", 
                     missing = "FIML",
                     meanstructure = T, 
...
```{r w1}
kable(param_d1[op=="~" & rhs %in% param_d1$rhs[c(25:28, 33:38)] & lhs %in% param_d1$lhs[c(25:28, 33:38)] , !"label"],
      digits = 3,
...
```{r w2}
kable(param_d2[op=="~" & rhs %in% param_d2$rhs[c(25:28, 33:38)] & lhs %in% param_d2$lhs[c(25:28, 33:38)] , !"label"],
      digits = 3,
      caption = "D2 model interpretations of within components") %>% 
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = T) %>% 
  row_spec(c(6,8,10), bold = T, color = "white", background = "#9CE599") # Añadir destacado
```

At the Table \@ref(tab:w2) estimates of the crosslagged regressions are showed for the constrained models. It is observed that a deviation of one point above the average level of trust in trade unions predicts a negative deviation of `r round(param_d2$est[param_d2$lhs == "DOM_INDEX4" & param_d1$rhs == "CS3"],3)` in SDO during the next measurement, controlling for deviations from the expected score in SDO and sex.
