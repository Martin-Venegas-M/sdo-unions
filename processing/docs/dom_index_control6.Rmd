...
title: |
  ...
  RI-CLPM Control Model 6 (Sex): SDO & Trust in Trade Unions
...
kable(param_d1[op=="~" & rhs %in% param_d1$rhs[c(33:41)] & lhs %in% param_d1$lhs[c(33:41)] , !"label"],
  ...
  row_spec(c(6), bold = T, color = "white", background = "#9CE599") # Añadir destacado
```

At the Table \@ref(tab:w1) estimates of the crosslagged regressions are showed for the unconstrained models for women and men. The analysis reveals a negative effect between SDO and trust in trade unions at waves 4, 3 and 2. In the case of men, a deviation of one point above the average level of trust in trade unions at wave 3 predicts a deviation of `r round(param_d1$est[param_d1$lhs == "DOM_INDEX4" & param_d1$rhs == "CS3"& param_d1$group == "Men"],3)` in SDO during wave 4, controlling for deviations from the expected score in SDO.

In the case of women, there are no statistical significance between SDO and trust in trade unions.
...
kable(param_d2[op=="~" & rhs %in% param_d2$rhs[c(33:41)] & lhs %in% param_d2$lhs[c(33:41)] , !"label"],
  ...
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = T) %>% 
  row_spec(c(2,4,6, 12, 14, 16), bold = T, color = "white", background = "#9CE599") # Añadir destacado
```

At the Table \@ref(tab:w2) estimates of the crosslagged regressions are showed for the constrained models. It is observed that, for men and women, a deviation of one point above the average level of trust in trade unions predicts a negative deviation of `r round(param_d2$est[param_d2$lhs == "DOM_INDEX4" & param_d2$rhs == "CS3" & param_d2$group == "Men"],3)` in SDO during the next measurement, controlling for deviations from the expected score in SDO.
