...
title: |
  ...
  CLPM(extendido) Basic Model: SDO & Trust in Trade Unions
...
knit: (function(inputFile, encoding) {
      out_dir <- "../../output/docs/CLPM-Basic-Model";
...
b1 <- '
    ...
    DOM_INDEX3 ~ DOM_INDEX2 + DOM_INDEX1 + CS2  
    
    DOM_INDEX4 ~ DOM_INDEX3 + DOM_INDEX2 + DOM_INDEX1 + CS3
...
b2 <- '
    ...
    DOM_INDEX3 ~ a*DOM_INDEX2 + a*DOM_INDEX1 + b*CS2
    
    DOM_INDEX4 ~ a*DOM_INDEX3 + a*DOM_INDEX2 + a*DOM_INDEX1 + b*CS3
...
c1 <- '
    ...
    CS3 ~ DOM_INDEX2 + CS2 + CS1
    
    CS4 ~ DOM_INDEX3 + CS3 + CS2 + CS1
'
...
c2 <- '
    ...
    CS2 ~ h*DOM_INDEX1 + i*CS1
    
    CS3 ~ h*DOM_INDEX2 + i*CS2 + i*CS1
    
    CS4 ~ h*DOM_INDEX3 + i*CS3 + i*CS2 + i*CS1
'
...
kable(param_b1[op=="~" & rhs %in% param_b1$rhs[c(17:31)] & lhs %in% param_b1$lhs[c(17:31)] , !"label"],
      digits = 3,
  ...
  row_spec(c(5,9), bold = T, color = "white", background = "#9CE599") # Añadir destacado
```
...
kable(param_b2[op=="~" & rhs %in% param_b2$rhs[c(17:31)] & lhs %in% param_b2$lhs[c(17:31)] , !"label"],
      digits = 3,
      caption = "B2 model interpretations of within components") %>% 
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = T) %>% 
  row_spec(c(2,5,9), bold = T, color = "white", background = "#9CE599") # Añadir destacado
```
...
kable(param_d1[op=="~" & rhs %in% param_d1$rhs[c(17:34)] & lhs %in% param_d1$lhs[c(17:34)] , !"label"],
      digits = 3,
      caption = "D1 model interpretations of within components") %>% 
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = T) %>% 
   row_spec(c(2,10,12,15), bold = T, color = "white", background = "#E37383") %>% 
  row_spec(c(5,9), bold = T, color = "white", background = "#9CE599") # Añadir destacado
```
...
kable(param_d2[op=="~" & rhs %in% param_d2$rhs[c(17:34)] & lhs %in% param_d2$lhs[c(17:34)] , !"label"],
      digits = 3,
      caption = "D2 model interpretations of within components") %>% 
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = T) %>% 
  row_spec(c(2,5,9), bold = T, color = "white", background = "#9CE599") %>% # Añadir destacado
row_spec(c(10,12,15), bold = T, color = "white", background = "#E37383")
```
