RICLPM.ext1 <- '
  ...
  DOM_INDEX1 + DOM_INDEX2 + DOM_INDEX3 + DOM_INDEX4 ~ s1*edad1 # Constrained over time. 
  CS1 + CS2 + CS3 + CS4 ~ s2*edad1 # Constrained over time. 
...
summary(RICLPM.ext1.fit, standardized = T, fit.measures = T)
